...
function Applicants() {
  ...
  async function fetchData() {
    const data = await applicantssApi() // Llama a la función para obtener los datos
    setApplicants(data.data) // Establece los datos de los solicitantes en el estado
  }
          ...
          <article className="p-0 grid xl:grid-cols-3 xl:gap-8 sm:grid-cols-2 sm:gap-4">
            {
              ...
              <div className="w-24 h-24 rounded-full mx-auto overflow-hidden">
                <img className="block w-flil h-flil object-cover" src={`../assets/img/${applicant.image}`} alt={`${applicant.name} ${applicant.lastName}`}/>
              </div>
